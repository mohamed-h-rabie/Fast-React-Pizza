...
function Button({ children, disabled, to, type, onClick }) {
    const base = `inline-block text-sm rounded-full bg-yellow-400 
         font-semibold uppercase tracking-wide 
          ...
          hover:bg-yellow-300 focus:bg-yellow-400 
    ...
    const styles = {
     ...
     tracking-wide  text-stone-400 transition-colors
      duration-300 hover:bg-stone-300 focus:bg-stone-400 
      focus:outline-none focus:ring focus:ring-stone-400
       focus:ring-offset-2 disabled:cursor-not-allowed  
        px-4 py-2.5 sm:px-6 sm:py-3.5 focus:text-stone-800 
        hover:text-stone-800 `,
        third: base + 'px-2 py-1',
    }
    if (to)
        return (
